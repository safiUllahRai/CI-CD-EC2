 ...
 */
export type NotEmpty<T, U = {
    [K in keyof T]: Pick<T, K>;
}> = Partial<T> & U[keyof U];
export type ExtractObservableValue<T> = T extends Observable<infer R> ? R : never;
export type PropName<T> = keyof T;
export type PropType<T> = T[PropName<T>];
/**
 * Typed reducer function for the `Array#reduce` method.
 */
export type ArrayReducerFn<T extends Record<string, any>> = (acc: T, cur?: PropType<T>, idx?: number) => T;
